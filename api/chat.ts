...
function getGemini(): GoogleGenAI {
  if (!aiClient) {
    ...
    if (!key) {
      throw new Error("GEMINI_API_KEY environment variable is required. Please set it.");
    }
...
export default async function handler(req: any, res: any) {
  // Support both GET (health check) and POST
  if (req.method === "GET") {
    return res.status(200).json({ status: "ok", message: "AIM Chat API is active" });
  }
